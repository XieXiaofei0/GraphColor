...
bool test_tabuSearch(string &file_name, int nb_color) {
    constexpr long long max_iter = 4500 * 1000;
...
bool test_crossOver(string &file_name, int nb_color) {
    constexpr long long max_iter = 300 * 1000;
    constexpr int max_population = 50;
    constexpr int population = 2;
...
void benchmark(bool test(string&, int)) {
    ...
    constexpr int max_num_calculations = 200;          //最大的计算次数
    ...
    {
        string file_name = "Instances/DSJC" + instances_id[7] + ".col";
        int nb_color = best_known_colors[7] + 1;
        mylog << "第" << num << "次测试" <<= LogSwitch(1, 1, "BenchMark");
        if (test(file_name, nb_color)) {
            mylog << "第" << num << "次测试冲突数为0" <<= LogSwitch(1, 1, "The num of conflicts is zero!!!!!");
            //break;
        }
    }
}
